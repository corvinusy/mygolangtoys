...
func main() {
	...
	for n1 = 1; n1 < LIMIT; n1++ {
		for n2 = n1 + 1; n2 < LIMIT; n2++ {
			if !is_satisfies(n1, n2, primes, prmap) {
				continue
			}
			for n3 = n2 + 1; n3 < LIMIT; n3++ {
				if !is_satisfies(n1, n3, primes, prmap) {
					continue
				}
				if !is_satisfies(n2, n3, primes, prmap) {
					continue
				}
				for n4 = n3 + 1; n4 < LIMIT; n4++ {
					if !is_satisfies(n1, n4, primes, prmap) {
						continue
					}
					if !is_satisfies(n2, n4, primes, prmap) {
						continue
					}
					if !is_satisfies(n3, n4, primes, prmap) {
						continue
					}
					for n5 = n4 + 1; n5 < LIMIT; n5++ {
						if !is_satisfies(n1, n5, primes, prmap) {
							continue
						}
						if !is_satisfies(n2, n5, primes, prmap) {
							continue
						}
						if !is_satisfies(n3, n5, primes, prmap) {
							continue
						}
						if !is_satisfies(n4, n5, primes, prmap) {
							continue
						}
